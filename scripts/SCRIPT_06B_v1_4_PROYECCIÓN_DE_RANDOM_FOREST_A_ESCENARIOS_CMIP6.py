...
class Config:
    ...
    def __post_init__(self):
        for c in [Path(r"D:\POSGRADOS\TESIS"), Path(r"D:/POSGRADOS/TESIS")]:
            if (c / "02_DATOS").exists():
                self.BASE_DIR = c
                break
        else:
            self.BASE_DIR = Path(r"D:\POSGRADOS\TESIS")

        for c in [Path(r"D:\POSGRADOS\TESIS\Prevención_de_Riesgos"),
